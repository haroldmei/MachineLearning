...
def get_sliding_box_detections(image_arr: np.ndarray) -> DetectionsSample:
    ...
    detections_sample = DetectionsSample(detections, 3)
    return _shuffle_sample(detections_sample)
...
def get_collide_detections(image_arr: np.ndarray) -> DetectionsSample:
    ...
    for i in range(detections_length):
        ...
        detections.append(detections_for_frame)
    detections_sample = DetectionsSample(detections, 2)
    return _shuffle_sample(detections_sample)
...
def get_saved_detections(image_arr: np.ndarray, sample_num: int) -> DetectionsSample:
    ...
    detections_sample = DetectionsSample(detections, raw_samples["expected_num_tracks"])
    return _shuffle_sample(detections_sample)


def _shuffle_sample(detections_sample: DetectionsSample) -> DetectionsSample:
    for ds in detections_sample.detections:
        random.shuffle(ds)
    return detections_sample
